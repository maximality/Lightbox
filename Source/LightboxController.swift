...
extension LightboxController: HeaderViewDelegate {
    ...
    func headerView(_ headerView: HeaderView, didPressShareButton closeButton: UIButton) {
        let initialMultiMedia = initialImages[currentPage]
        var activityItems = [Any]()
        if initialMultiMedia.type == "image" && backgroundView.image != nil {
            activityItems.append(backgroundView.image!)
        } else if initialMultiMedia.type == "gif" {
            let urlString = initialMultiMedia.videoURL?.absoluteString ?? initialMultiMedia.imageURL?.absoluteString ?? ""
            activityItems.append(urlString)
        } else {
            let videoURL = initialMultiMedia.videoURL?.absoluteString ?? initialMultiMedia.imageURL?.absoluteString ?? ""
